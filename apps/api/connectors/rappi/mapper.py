...
def map_rappi_order_to_internal(payload: dict, tenant, store) -> dict:
    """Map Rappi order payload to internal Order dict.

    Does NOT persist anything — pure transformation.

    Rappi fields:
    - id, displayId, store_id
    ...
    display_number = order_data.get("displayId") or external_id[:8]

    return {
        "tenant": tenant,
        "store": store,
        "channel": OrderChannel.RAPPI,

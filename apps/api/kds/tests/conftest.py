...
from .factories import KDSStationFactory, KDSTicketFactory
...
def auth_client(api_client, user, store):
    ...
    api_client.store = store
    # Store the store ID so it's sent with every request via a wrapper
...
@pytest.fixture
def kds_station_factory(store):
    def _create(**kwargs):
        kwargs.setdefault("store", store)
        return KDSStationFactory(**kwargs)

    return _create


@pytest.fixture
def kds_ticket_factory(kds_station_factory):
    def _create(**kwargs):
        if "station" not in kwargs:
            kwargs["station"] = kds_station_factory()
        station = kwargs["station"]
        if "order" not in kwargs:
            kwargs["order"] = OrderFactory(
                store=station.store,
                tenant=station.store.company.tenant,
            )
        return KDSTicketFactory(**kwargs)
